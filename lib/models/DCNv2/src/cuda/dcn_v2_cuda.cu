...
#ifndef THArgCheck
#define THArgCheck(cond, arg, msg) TORCH_CHECK((cond), msg)
#endif
#include <cublas_v2.h>
                    ...
                    const int deformable_group)
{
    ...
    auto input_b = static_cast<const float **>(cudaMalloc(state, matrices_size));
    auto output_b = static_cast<float **>(cudaMalloc(state, matrices_size));
    auto columns_b = static_cast<float **>(cudaMalloc(state, matrices_size));
    auto ones_b = static_cast<const float **>(cudaMalloc(state, matrices_size));
    auto weight_b = static_cast<const float **>(cudaMalloc(state, matrices_size));
    auto bias_b = static_cast<const float **>(cudaMalloc(state, matrices_size));

    const int block = 128;
    const int grid = (batch + block - 1) / block;

    createBatchGemmBuffer<<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(
        input_b, output_b,
        columns_b, ones_b,
        weight_b, bias_b,
        input.data<scalar_t>(),
    ...
    long k_ = 1;
    cublasSgemmBatched(get_blas_handle(),
                            't',
                            'n',
                            n_,
                            m_,
    ...
    cudaFree(state, input_b);
    cudaFree(state, output_b);
    cudaFree(state, columns_b);
    cudaFree(state, ones_b);
    cudaFree(state, weight_b);
    cudaFree(state, bias_b);
    return output;
}
                                             ...
                                             int deformable_group)
{
    ...
    for (int b = 0; b < batch; b++)
    {
        ...
        cublasSgemm(get_blas_handle(), 'n', 't', n, m, k, 1.0f,
