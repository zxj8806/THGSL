...
def trainer_decode(heat, wh, reg=None, tracking=None, pre_inds=None, num_classes=1, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()
    heat = _nms(heat)
    ret = {}
    scores, inds, clses, ys, xs, cls_inds_masks = _topk(heatmap=heat, K=K, num_classes=num_classes)
    if reg is not None:
      reg = _transpose_and_gather_feat(reg, inds)
      reg = reg.view(batch, K, 2)
      xs = xs.view(batch, K, 1) + reg[:, :, 0:1]
